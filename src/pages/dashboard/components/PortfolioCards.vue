<template>
    <transition name="slide-fade" mode="out-in">
        <NotificationBar v-if="isThereNoPortfolioFile" :key="isThereNoPortfolioFile"
...
export default {
    ...
    computed: {    
        ...
        isThereData() {
            return !!this.currentPortfolio.portfolioFile && !!this.currentPortfolio.accountFile;
        },
        ...
        totalInvestedAmount() {
            if(this.isThereData) {
                let tot = this.getTotalInvestedAmount(this.currentPortfolio.portfolioFile);
                return this.currencyMarkup(tot);
            }
            return 0;
        },
        investedPercentage() {
            if(this.isThereData) {
                let totInvested = this.getTotalInvestedAmount(this.currentPortfolio.portfolioFile);
                let totBalance = this.getTotalBalance(this.currentPortfolio.portfolioFile);
        ...
        totalProfitLoss() {
            if(this.isThereData) {
                let totProfitLoss = this.getTotalProfitLoss(this.currentPortfolio.portfolioFile, this.currentPortfolio.accountFile);
