<template>
  <td>
    <span class="cutText">
      <router-link
        class="holdingName"
        :to="
          '/dashboard/' + this.$route.params.id + '/holdings/' + holding.isin
        "
      >
        {{ holding.name }}
        <Icon
  ...
  <td class="number" :class="{ sellRedNumber: !isGain, buyGreenNumber: isGain }">
    <span>
      {{
        Intl.NumberFormat("nl-nl", {
          style: "currency",
    ...
    <br />
    <span class="secondary" :class="{ sellRedNumber: !isGain, buyGreenNumber: isGain }" v-if="holding.costBasis > 0">
      {{
        Intl.NumberFormat("nl-nl", {
          style: "percent",
